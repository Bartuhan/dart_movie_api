    ...
    return (Request request) async {
      ...
      if (query == "me") {
        if (request.context['authDetails'] == null) {
          return Response.forbidden(json.encode({'error': 'Not authorized to perform this action'}), headers: CustomHeader.json.getType);
        }
        final userId = formatUserId((request.context['authDetails'] as JWT).subject ?? '');

        if (userId.isNotEmpty) {
          final currUser = await store.findOne(where.eq('_id', ObjectId.fromHexString(userId)));
...
String formatUserId(String userId) {
  ...
  String hexString = userId.substring(startIndex, endIndex);
